...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    const userDataResponse = await fetch(`https://oauth2.googleapis.com/tokeninfo?accessToken=${googleToken}`);
    const userData = await userDataResponse.json();

    let dbid;
    const [user] = await db.execute('SELECT * FROM users WHERE email = ?', [userData.email]);
    
    if (user.length > 0) {
      dbid = user[0].id;
    } else {
      return res.status(400).json({ message: 'Naudotojas su tokiu el. paštu neegzistuoja.' });
    }
