...
function ViewCourse() {
    ...
    const handleEditConfirm = async () => {
        if (courseTitle === course?.name && courseDescription === course?.description) {
            setEditActive(false);
            return;
        }
        ...
        try {
            const res = await fetch(`http://localhost:5000/course/update`, {
                ...
                body: JSON.stringify({ id, name: courseTitle, description: courseDescription, icon_url: "" })
            });
    
            if (!res.ok) throw new Error(`HTTP error: ${res.status}`);

            const updatedCourse = { ...course, name: courseTitle, description: courseDescription, icon_url: courseIcon };
                    ...
                    <>
                        ...
                        <div>
                            <img 
                                ...
                                style={{ 
                                    cursor: courseTitle === course?.name && courseDescription === course?.description 
                                        ? "not-allowed" 
                                        : "pointer", 
                                    opacity: courseTitle === course?.name && courseDescription === course?.description 
                                        ? 0.5 
                                        : 1 
                                }}
                            />
